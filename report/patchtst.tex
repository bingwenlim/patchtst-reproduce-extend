...
Table~\ref{tab:acca_main} reports the main ACCA result across all four datasets. Both ACCA variants track the CI baseline to within the third decimal place on every dataset: the absolute MSE change never exceeds 0.6\%, and on Traffic, Air, and FX the change is entirely within epoch-to-epoch optimizer noise. The learned gate converges to $\alpha_{\text{effective}}$ between 0.01 and 0.03---very close to its initialization and near zero---on every run, confirming that gradient descent actively chooses \textit{not} to open the cross-channel path. This holds even on FX, the dataset deliberately chosen for its strong cross-currency correlations. Figure~\ref{fig:mse_delta} visualizes the per-run relative change versus the CI baseline.
...
\multirow{3}{*}{Traffic}
 ...
 & + ACCA (attn,   pre) & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
\midrule
\multirow{3}{*}{Air}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\linewidth]{figures/mse_delta.pdf}
\caption{Test-MSE change of each ACCA ablation run relative to the CI PatchTST baseline, per dataset. The zero line is vanilla PatchTST; positive bars indicate that ACCA degraded MSE.}
\label{fig:mse_delta}
\end{figure}
...
\textbf{Placement.} Post-head mixing collapses the prediction to a weighted average of the per-channel forecasts and never beats the pre-head variant or the CI baseline. In the linear post-head setting the learned gate remains near zero, mirroring the pre-head behavior; the attention post-head variant is noticeably slower (mixing is applied on longer $H$-dimensional vectors) with no measurable MSE gain.

\textbf{Mixing operator.} The attention operator never outperforms the linear operator by a meaningful margin, even on FX where we expected its data-dependent weighting to matter. The gap between \texttt{linear} and \texttt{attention} is within 0.003 MSE across all datasets, so the extra $O(D)$-factor parameter count of the attention variant yields no measurable return.

\textbf{Gate mode.} Forcing $\alpha = 1$ (\texttt{fixed\_one}) lets us reject the hypothesis that the learned gate alone is responsible for ACCA's silence. In this mode the prediction head is trained exclusively on the channel-mixed representation, so if cross-channel context genuinely helped we would see lower MSE. Instead, \texttt{fixed\_one} \textit{worsens} MSE on ETTh1: the attention variant degrades by 4.4\%, while the bare linear variant collapses by 53.4\%. The asymmetry is informative: the attention module retains a residual connection and a LayerNorm, so even when fully gated in it still lets most of the encoder representation through; the linear variant replaces the encoder output with a data-independent linear combination of channels and destroys the temporal structure the encoder extracted. Both outcomes point in the same direction -- cross-channel mixing at this stage is \textit{strictly harmful} information-theoretically, and the gate correctly identifies it as such. The FX gate-mode ablation (rows with \textit{TBD}) awaits GPU compute and is expected to show a similar, though likely milder, asymmetry given the weaker residual path.
...
\multirow{4}{*}{FX}
 ...
 & linear, post-head  & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
 & attn,   pre-head   & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
 & attn,   post-head  & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
\bottomrule
\end{tabular}
\end{table}
...
\multirow{4}{*}{FX}
 ...
 & linear & fixed\_one  & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
 & attn   & learned     & \textit{TBD} & \textit{TBD} & -- \\
 & attn   & fixed\_one  & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\linewidth]{figures/alpha_trace.pdf}
\caption{Per-epoch evolution of $\alpha_{\text{effective}} = \sigma(\alpha_{\text{raw}})$ for every ACCA ablation run (symlog $y$-axis). Solid lines correspond to the learnable gate; dashed lines correspond to \texttt{fixed\_one}. All learned runs settle near the initialization value of $0.01$, meaning the gate actively chooses to stay closed.}
...
The four ablations jointly rule out every hypothesis one might form to explain ACCA's silence as an artifact of a specific design choice. The learned gate is not the culprit (the \texttt{fixed\_one} variant is strictly worse); the mixing operator is not too weak (the more expressive attention form behaves identically to the linear form); the injection placement is not wrong (pre-head and post-head agree). The consistent conclusion is that, under the paper's CI training regime and forecast horizon $H{=}96$, cross-channel information is effectively redundant with the information already extracted independently by the shared backbone.

This result is consistent with but stronger than the observation made in DLinear~\cite{dlinear}: Zeng et al.\ demonstrated that a single linear layer is competitive with Transformer-based forecasters on standard benchmarks; our results show that adding cross-channel capacity \textit{also} does not help, even at the representation level. It also complements recent architectural lines of work: Crossformer~\cite{crossformer} and iTransformer~\cite{itransformer} \textit{do} report cross-channel gains, but only after redesigning the entire backbone to put channels on the token axis. In contrast, the TSMixer family~\cite{tsmixer} decouples time-mixing and channel-mixing as separate residual MLP blocks and also benefits from explicit channel mixing. The divergence suggests that the CI backbone of PatchTST is not simply ignoring channel information; it is actively encoding a representation in which cross-channel mixing is already implicit, so a late-stage mixing layer has no additional lift to provide.

A second interpretation of the FX result is that strong cross-variable correlation does not automatically mean high cross-variable \textit{predictability}. In an efficient-markets setting, currency pairs move together because they share an unpredictable driver (global risk sentiment), not because one currency is predictive of another's future value. If the cross-channel signal is itself noise-dominated, cross-channel mixing only passes noise between variables---a plausible explanation for the mild \textit{degradation} we see in the \texttt{fixed\_one} ablation.
