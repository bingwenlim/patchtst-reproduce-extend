...
\textbf{Gate mode.} Forcing $\alpha = 1$ (\texttt{fixed\_one}) lets us reject the hypothesis that the learned gate alone is responsible for ACCA's silence. In this mode the prediction head is trained exclusively on the channel-mixed representation, so if cross-channel context genuinely helped we would see lower MSE. Instead, \texttt{fixed\_one} consistently \textit{worsens} MSE on both ETTh1 and FX, confirming that the gate correctly identifies cross-channel mixing as harmful for the chosen forecast horizon.
