...
\caption{Test-MSE change of each ACCA ablation run relative to the CI PatchTST baseline, per dataset (shown here for ETTh1; full numerical results for Traffic, Air, and FX are in Tables~\ref{tab:acca_main}--\ref{tab:acca_alpha}). The zero line is vanilla PatchTST; positive bars indicate that ACCA degraded MSE.}
\label{fig:mse_delta}
\end{figure}
...
\caption{Per-epoch evolution of $\alpha_{\text{effective}} = \sigma(\alpha_{\text{raw}})$ for the five ETTh1 ACCA ablation runs (symlog $y$-axis). Solid lines correspond to the learnable gate; dashed lines correspond to \texttt{fixed\_one}. All learned runs settle near the initialization value of $0.01$, meaning the gate actively chooses to stay closed. The qualitative behavior on Traffic, Air, and FX is identical: see the $\alpha_{\text{eff}}$ columns of Tables~\ref{tab:acca_main}--\ref{tab:acca_alpha}, all in the range $[0.0098, 0.0132]$.}
