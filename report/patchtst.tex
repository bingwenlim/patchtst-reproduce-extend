...
To evaluate the generalization capability of PatchTST beyond the ETTh1 benchmark, we conducted experiments on additional datasets with diverse statistical properties. In particular, we focused on datasets with strong cross-channel dependencies and weak temporal regularity, which present a challenging setting for models that rely on local temporal patterns and channel-independent representations.

\bigskip

\noindent Specifically, we consider three datasets:
\begin{itemize}
\item \textbf{Traffic Dataset:} [PLACEHOLDER]
\item \textbf{Air Dataset:} [PLACEHOLDER]
...
\subsection{Additional Benchmark Dataset 2: Air Dataset}
