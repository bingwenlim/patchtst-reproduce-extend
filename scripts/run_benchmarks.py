...
models_configs = [
    ...
    { "model": "PatchTST", "args": ["--d_model", "16", "--n_heads", "4", "--d_ff", "128", "--dropout", "0.3", "--use_acca", "--acca_type", "linear"], "name": "PatchTST (ACCA linear)" },
...
with open(md_path, "w", encoding="utf-8") as f:
    ...
    for dataset, rows in grouped.items():
        f.write(f"## {dataset}\n\n")
        f.write("| Model | Test MSE | Test MAE | Best Epoch | Time |\n")
        f.write("| --- | --- | --- | --- | --- |\n")
        for row in rows:
            f.write(
                f"| {row['model']} | {row['test_mse']} | {row['test_mae']} | "
                f"{row['best_epoch']} | {row['time']} |\n"
            )
        f.write("\n")
